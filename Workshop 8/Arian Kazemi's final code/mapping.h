...
/** Arian Kazemi By adding the struct Shipment in the header file (mapping.h)
we make it accessible to other files that include this header.
This allows other parts of the program to use and manipulate shipments.
 * A shipment represents a package that needs to be delivered.
 */
struct Shipment
{
	int weight;        // Weight of the shipment in kilograms
	double size;       // Size of the box required in cubic meters
	struct Point dest; // Destination of the box (building position)
};
/**
...
/** Arian Kazemi Function declarations specify the names, parameters, and return types of functions that are defined elsewhere.
They act as a contract, informing other parts of the program about the existence and interface of these functions.
 * Determine the truck that can accommodate the shipment and has the shortest path to the destination.
 * @param baseMap - the base map with buildings
 * @param blueRoute - the route for blue trucks
 * @param greenRoute - the route for green trucks
 * @param yellowRoute - the route for yellow trucks
 * @param shipment - the shipment to be assigned to a truck
 * @returns - the route symbol of the truck assigned to the shipment (B, G, or Y)
 */
char assignShipmentToTruck(const struct Map* baseMap, const struct Route* blueRoute, const struct Route* greenRoute, const struct Route* yellowRoute, const struct Shipment* shipment);

/**
 * Print the shortest path from a truck's nearest point to the destination.
 * @param baseMap - the base map with buildings
 * @param route - the truck's route
 * @param nearestPoint - the nearest point on the route to the destination
 * @param dest - the destination point
 */
void printPathToDestination(const struct Map* baseMap, const struct Route* route, const struct Point* nearestPoint, const struct Point* dest);
